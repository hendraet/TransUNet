...
def generate_snapshot_path(args, dataset_name):
    ...
    snapshot_path = os.path.join(snapshot_path, 'best_model.pth')
    if not os.path.exists(snapshot_path):
        snapshot_path = snapshot_path.replace('best_model', 'epoch_' + str(args.max_epochs - 1))
...
def main(args):
    ...
    if args.snapshot_path is None:
        snapshot_path = generate_snapshot_path(args, dataset_name)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-sp', '--snapshot_path', type=str,
                        help='Path to the best model. If this is not set, the path will be generated from the other '
                             'parameters as in the training script.')
